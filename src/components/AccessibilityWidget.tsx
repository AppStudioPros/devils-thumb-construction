...
function A11yIcon({ size = 28 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" aria-hidden="true">
      <defs>
        <radialGradient id="dtcCircleGrad" cx="38%" cy="32%" r="65%">
          <stop offset="0%" stopColor="#2c4b40" />
          <stop offset="55%" stopColor="#13251e" />
          <stop offset="100%" stopColor="#0a1510" />
        </radialGradient>
        <radialGradient id="dtcFigureGrad" cx="40%" cy="30%" r="65%">
          <stop offset="0%" stopColor="#f5c84a" />
          <stop offset="100%" stopColor="#e09f18" />
        </radialGradient>
      </defs>
      <circle cx="24" cy="24" r="22" fill="url(#dtcCircleGrad)" />
      <circle cx="24" cy="24" r="22" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5" />
      <circle cx="24" cy="13" r="3.5" fill="url(#dtcFigureGrad)" />
      <rect x="22.5" y="17" width="3" height="9" rx="1.5" fill="url(#dtcFigureGrad)" />
      <rect x="13" y="18.5" width="22" height="3" rx="1.5" fill="url(#dtcFigureGrad)" />
      <rect x="22.5" y="25.5" width="3" height="9.5" rx="1.5" transform="rotate(-15 22.5 25.5)" fill="url(#dtcFigureGrad)" />
      <rect x="22.5" y="25.5" width="3" height="9.5" rx="1.5" transform="rotate(15 25.5 25.5)" fill="url(#dtcFigureGrad)" />
...
export default function AccessibilityWidget() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('dtc-a11y-settings')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('dtc-a11y-settings', JSON.stringify(settings))
  }, [settings])

  const reset = () => setSettings(DEFAULT)

  const btnStyle = (active: boolean): React.CSSProperties => ({
    background: active ? '#e09f18' : 'rgba(255,255,255,0.08)',
    border: `1px solid ${active ? '#e09f18' : 'rgba(255,255,255,0.15)'}`,
    color: active ? '#13251e' : 'rgba(255,255,255,0.75)',
  ...
  return (
    <>
      ...
      <button onClick={() => setOpen(!open)} aria-label="Accessibility options" aria-expanded={open}
        style={{ position:'fixed', bottom:96, right:24, zIndex:9998, width:40, height:40, borderRadius:'50%', background:'transparent', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', padding:0, filter:'drop-shadow(0 4px 12px rgba(0,0,0,0.35))', transition:'transform 0.15s' }}
      ...
      {open && (
        <div role="dialog" aria-label="Accessibility settings" aria-modal="true"
          style={{ position:'fixed', bottom:152, right:24, zIndex:9997, width:300, background:'#13251e', border:'1px solid rgba(224,159,24,0.35)', borderRadius:10, boxShadow:'0 12px 40px rgba(0,0,0,0.45)', padding:20, color:'#fff' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
            ...
            <div style={{ display:'flex', gap:6 }}>
              <button onClick={reset} title="Reset all" style={{ background:'none', border:'none', color:'rgba(255,255,255,0.5)', cursor:'pointer', padding:4 }}><RotateCcw size={14} /></button>
              <button onClick={() => setOpen(false)} aria-label="Close" style={{ background:'none', border:'none', color:'rgba(255,255,255,0.5)', cursor:'pointer', padding:4 }}><X size={16} /></button>
            </div>
          </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>
              {fontSizes.map(({ label, size, title }) => (
                <button key={size} onClick={() => setSettings(p => ({ ...p, fontSize: size }))} title={title}
                  style={btnStyle(settings.fontSize === size)} aria-pressed={settings.fontSize === size}>{label}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            ...
            {([
              ...
              <button key={key} onClick={() => setSettings(p => ({ ...p, [key]: !p[key] }))}
                style={{ ...btnStyle(settings[key] as boolean), justifyContent:'space-between' }} aria-pressed={settings[key] as boolean}>
                <span>{label}</span>
                <span style={{ width:32, height:18, borderRadius:9, background:settings[key] ? '#e09f18' : 'rgba(255,255,255,0.15)', display:'flex', alignItems:'center', padding:'0 3px', transition:'background 0.15s' }}>
          ...
          <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid rgba(255,255,255,0.08)', textAlign:'center' }}>
            <a href="/accessibility" style={{ color:'#e09f18', fontSize:'0.78rem', textDecoration:'underline' }}>Accessibility Statement</a>
          </div>
        </div>
      )}
    </>
